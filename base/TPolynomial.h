...
public:
	...
	TPolinomial operator-(const TPolinomial& pol) const  { // не чекал
		Node* p1 = pFirst, * p2 = pol.pFirst;
		Node* npFirst = new Node();
		Node* pcur = npFirst;
		while (p1 != nullptr || p2 != nullptr) {
	...
	TPolinomial operator*(double c) const { // јјјјјјјјјјјјј может изенить double на int ( если коэфы double )
	...
	friend TPolinomial operator*(double c, const TPolinomial& pol) { // јјјјјјјјјјјјј мб не дабл?
		return (pol * c);
	}
